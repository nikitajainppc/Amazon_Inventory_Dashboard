...
if uploaded_file:
    ...
    status_filter = st.multiselect("🔍 Filter by Inventory Status", options=df['inventory_status'].unique(), default=df['inventory_status'].unique())
    filtered_df = df[df['inventory_status'].isin(status_filter)]

    st.subheader("📦 Filtered Inventory Report")
    st.dataframe(filtered_df)

    csv_export = filtered_df.to_csv(index=False).encode('utf-8')
    st.download_button("📥 Download Filtered Report", csv_export, file_name="Filtered_Inventory_Report.csv", mime='text/csv')
else:
